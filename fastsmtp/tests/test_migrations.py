...
def _run_alembic(database_url: str, *args: str) -> subprocess.CompletedProcess[str]:
    ...
    ``alembic/env.py`` takes the URL from ``get_settings()``, which is cached for
    the life of the process, and drives the migrations with ``asyncio.run``.
    Both make it hostile to in-process invocation from an async test; a child
    process also matches how migrations are applied for real.
    """
    return subprocess.run(
        [sys.executable, "-m", "alembic", *args],
        cwd=PACKAGE_ROOT,
        env={**os.environ, "FASTSMTP_DATABASE_URL": database_url},
        capture_output=True,
        text=True,
    )


async def alembic(database_url: str, *args: str) -> None:
    """Run an alembic command off the event loop, failing on a non-zero exit."""
    result = await to_thread.run_sync(partial(_run_alembic, database_url, *args))
...
async def test_migration_chain_is_reversible(migration_database_url: str) -> None:
    ...
    """
    await alembic(migration_database_url, "upgrade", "head")
    await alembic(migration_database_url, "downgrade", "base")
...
async def test_migrated_schema_matches_models(migration_database_url: str) -> None:
