...
async def lifespan(app: FastAPI):
    ...
    # Startup. Refuse a database older than this build, or one holding
    # encrypted columns this process has no key for, rather than discovering
    # either on the first query - see db/startup.py, which ``fastsmtp serve``
    # runs too so that every component is covered, not only this one.
    #
    # The settings are the app's own, not the process-wide get_settings():
    # create_app() accepts an override and the test suite uses it, so the
    # shared engine can point somewhere else entirely. Checking the wrong
    # database would be worse than not checking.
